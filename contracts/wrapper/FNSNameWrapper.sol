...
import {IFNSNameWrapper, INameWrapperUpgrade, CANNOT_UNWRAP, CANNOT_BURN_FUSES, CANNOT_TRANSFER, CANNOT_SET_RESOLVER, CANNOT_SET_TTL, CANNOT_CREATE_SUBDOMAIN, CANNOT_APPROVE, PARENT_CANNOT_CONTROL, CAN_DO_EVERYTHING, IS_DOT_FRAX, CAN_EXTEND_EXPIRY, PARENT_CONTROLLED_FUSES, USER_SETTABLE_FUSES} from "./IFNSNameWrapper.sol";
    ...
    ReverseClaimer
{
    ...
    bytes32 private constant FRAX_LABELHASH =
        0xe16c4e629aeb6288a6a8eb2633ed700071ada1af6ba75e9b0af9b702347ef259;
    ...
    constructor(
        ...
        /* Burn PARENT_CANNOT_CONTROL and CANNOT_UNWRAP fuses for ROOT_NODE and FRAX_NODE and set expiry to max */
        _setData(
            uint256(FRAX_NODE),
            address(0),
        ...
        names[ROOT_NODE] = "\x00";
        names[FRAX_NODE] = "\x04frax\x00";

        /* Set delegation for FXTL Points */
    ...
    function supportsInterface(
        ...
        view
        virtual
        override(ERC1155Fuse, IFNSNameWrapper)
        returns (bool)
    {
        return
            interfaceId == type(IFNSNameWrapper).interfaceId ||
    ...
    function approve(
        address to,
        uint256 tokenId
    ) public override(ERC1155Fuse, IFNSNameWrapper) {
    ...
    function uri(
        ...
        public
        view
        override(IFNSNameWrapper, IERC1155MetadataURI)
    ...
    function canModifyName(
        ...
        return
            ...
            !_isFRAX2LDInGracePeriod(fuses, expiry);
    }
    ...
    /**
     * @notice Wraps a .frax domain, creating a new token and sending the original ERC721 token to this contract
     * @dev Can be called by the owner of the name on the .frax registrar or an authorised caller on the registrar
     * @param label Label as a string of the .frax domain to wrap
    ...
    function wrapFRAX2LD(
        ...
        if (
            ...
            revert Unauthorised(
                _makeNode(FRAX_NODE, bytes32(tokenId)),
                msg.sender
            );
        }
    ...
    /**
     * @dev Registers a new .frax second-level domain and wraps it.
     *      Only callable by authorised controllers.
     * @param label The label to register (Eg, 'foo' for 'foo.frax').
    ...
    /**
     * @notice Renews a .frax second-level domain.
     * @dev Only callable by authorised controllers.
     * @param tokenId The hash of the label to register (eg, `keccak256('foo')`, for 'foo.frax').
     * @param duration The number of seconds to renew the name for.
     * @return expires The expiry date of the name on the .frax registrar, in seconds since the Unix epoch.
     */
    ...
    function renew(
        ...
        bytes32 node = _makeNode(FRAX_NODE, bytes32(tokenId));
    ...
    /**
     * @notice Wraps a non .frax domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
    ...
    /**
     * @notice Unwraps a .frax domain. e.g. sam.frax
     * @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
     * @param labelhash Labelhash of the .frax domain
     * @param registrant Sets the owner in the .frax registrar to this address
     * @param controller Sets the owner in the registry to this address
     */

    function unwrapFRAX2LD(
        bytes32 labelhash,
        address registrant,
        address controller
    ) public onlyTokenOwner(_makeNode(FRAX_NODE, labelhash)) {
        if (registrant == address(this)) {
            revert IncorrectTargetOwner(registrant);
        }
        _unwrap(_makeNode(FRAX_NODE, labelhash), controller);
    ...
    /**
     * @notice Unwraps a non .frax domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
     * @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
        ...
        )
    {
        ...
        if (owner == address(0)) {
            ...
            if (fuses & IS_DOT_FRAX == IS_DOT_FRAX) {
    ...
    function onERC721Received(
        ...
        _wrapFRAX2LD(label, owner, ownerControlledFuses, expiry, resolver);
    ...
    function _beforeTransfer(
        ...
        // For this check, treat .frax 2LDs as expiring at the start of the grace period.
        if (fuses & IS_DOT_FRAX == IS_DOT_FRAX) {
            expiry -= GRACE_PERIOD;
        }
    ...
    function _wrapFRAX2LD(
        ...
        bytes32 labelhash = keccak256(bytes(label));
        bytes32 node = _makeNode(FRAX_NODE, labelhash);
        // hardcode dns-encoded frax string for gas savings
        bytes memory name = _addLabel(label, "\x04frax\x00");
        names[node] = name;
        ...
        _wrap(
            ...
            name,
            wrappedOwner,
            fuses | PARENT_CANNOT_CONTROL | IS_DOT_FRAX,
            expiry
        );
    ...
    function _isFRAX2LDInGracePeriod(
        uint32 fuses,
        uint64 expiry
    ) internal view returns (bool) {
        return
            fuses & IS_DOT_FRAX == IS_DOT_FRAX &&
