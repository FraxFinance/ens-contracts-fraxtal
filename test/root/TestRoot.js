const Root = artifacts.require('./FNSRoot.sol')
...
contract('Root', function (accounts) {
  ...
  beforeEach(async function () {
    ...
    root = await Root.new(ens.address, FRAXTAL_DEL_REG, FRAXTAL_INITIAL_DEL)

    await root.setController(accounts[0], true)
    await ens.setSubnodeOwner('0x0', '0x' + sha3('frax'), root.address, {
      from: accounts[0],
    })
  ...
  describe('setSubnodeOwner', async () => {
    it('should allow controllers to set subnodes', async () => {
      await root.setSubnodeOwner('0x' + sha3('frax'), accounts[1], {
        from: accounts[0],
      })
    ...
    it('should fail when non-controller tries to set subnode', async () => {
      await exceptions.expectFailure(
        root.setSubnodeOwner('0x' + sha3('frax'), accounts[1], {
          from: accounts[1],
        }),
      )
    })
